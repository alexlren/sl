...
our $logFunctions = qr{(?x:
	uprintf(?:_ratelimited|_once|)|
	(?:[a-z0-9]+_){1,2}(?:uprintf|emerg|alert|crit|err|warning|warn|notice|log|info|debug|dbg|vdbg|devel|cont|WARN)(?:_ratelimited|_once|)|
...
sub build_types {
	...
	$Declare	= qr{(?:$Storage\s+(?:$Inline\s+)?)?(?:volatile\s+)?$Type(?:volatile\s+)?};
...
# check for redundant bracing round if etc
		...
		if (!defined $suppress_ifbraces{$linenr - 1} &&
			...
			if ($level == 0 && $block !~ /^\s*\{/ && !$allowed) {
				...
				WARN("BRACES_SINGLE",
				     "braces {} are always necessary for single statement blocks\n" . $herectx);
			}
		}
